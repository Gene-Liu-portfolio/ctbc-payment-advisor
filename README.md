...
- **1. Web Demo**：下載專案後只啟動前端，前端會連到已部署的 Render 服務。
...
不需要。Web Demo 預設連到已部署的 Render 服務；Cursor 或其他 MCP client 也只需要設定 `/mcp` URL。使用者自己的 agent / client 負責對話與推理，本服務只提供卡片資料、推薦、比較與優惠查詢工具。
